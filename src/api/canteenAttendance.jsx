...
const callProcedure = async (procedureName, params = {}) => {
  try {
    ...
    const response = await fetch(CanteenAttendanceCallProcedureURL, {
      method: 'POST',
 ...
 */
export const saveCanteenRegistration = async (registrationData) => {
  try {
    const {
      argEmpNo,
      argRegType,
    ...
    const data = await callProcedure('SMT_SAVE_CAN_REG', {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('Error saving canteen registration:', error);
    }
    return {
      success: false,
      data: null,
...
export const getCanteenRegistration = async ({
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('Error getting canteen registration:', error);
    }
    return {
      success: false,
      data: null,
